  ...
  return (
    <div className="bg-white">
      ...
      <motion.section
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="bg-linear-to-r from-primary to-primary-dark py-20"
      >
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          >
            ...
            <p className="text-white/90 text-xl max-w-2xl mx-auto mb-10">
              Access research journals, newsletters, reports, and professional development materials
            </p>
            <div className="max-w-2xl mx-auto">
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-6 py-4 rounded-xl outline-none text-lg bg-white placeholder:text-[#617289] transition-all focus:ring-2 focus:ring-primary focus:ring-offset-1"
                />
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { title: 'Research & Journals', icon: 'science', count: '45+ Papers', color: 'bg-primary' },
            { title: 'Newsletters', icon: 'newspaper', count: '120+ Issues', color: 'bg-primary' },
            { title: 'Reports & Fact Sheets', icon: 'summarize', count: '30+ Documents', color: 'bg-primary' },
            { title: 'CPD Materials', icon: 'school', count: '200+ Resources', color: 'bg-primary' },
          ].map((cat, idx) => (
            <motion.div
              key={idx}
            ...
            >
              <div className={`size-16 ${cat.color} rounded-2xl flex items-center justify-center text-white mb-6 group-hover:scale-110 transition-transform`}>
                <span className="material-symbols-outlined text-3xl">{cat.icon}</span>
              </div>
              <h3 className="text-2xl font-black mb-2">{cat.title}</h3>
              <p className="text-[#617289] font-bold">{cat.count}</p>
            </motion.div>
          ))}
        </div>
      </section>
